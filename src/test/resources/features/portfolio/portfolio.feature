Feature: Portfolio data read from server
  ...
  Scenario: Guest pulls default portfolio
    When "POST" request is sent to endpoint "/cv" with no body
  ...
  Scenario: Admin pulls specific portfolio
    ...
    When "POST" request is sent to endpoint "/cv/2" with no body
    Then Response status code should be 200
    And Response body should be:
    """
    {
  ...
  Scenario: Admin user pulls non-existent portfolio
    Given User is authorized with token: "admin"
    When "POST" request is sent to endpoint "/cv/999" with no body
