...
class EducationValidator(
  ...
  override fun validate(value: Education): ValidationResult {
    val result = listOf(
      ...
      validationFunc(
        value,
        { edu -> edu.fieldOfStudy.isNotEmpty() },
        "Field of study must not be empty"
      ),
      validationFunc(
        value,
        { edu -> edu.institution.name.isNotEmpty() },
        "Institution name must not be empty"
      ),
      validationFunc(
        value,
        { edu -> edu.institution.country.isNotEmpty() },
        "Institution country must not be empty"
      ),
      validationFunc(
        value,
        { edu -> edu.institution.city.isNotEmpty() },
        "Institution city must not be empty"
      ),
