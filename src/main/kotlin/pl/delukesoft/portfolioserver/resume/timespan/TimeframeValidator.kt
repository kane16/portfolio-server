package pl.delukesoft.portfolioserver.resume.timespan
...
class TimeframeValidator(
  ...
  override fun validate(value: Timeframe): ValidationResult {
    ...
    return if (validations.all { it.isValid }) ValidationResult.build() else ValidationResult.build(
      validations.flatMap { it.errors })
  }
  ...
  override fun validateList(values: List<Timeframe>): ValidationResult {
    val validations = listOf(
      validationListFunc(
        values.dropLast(1),
        ::timeframeIsNotOpenEnded,
        "Only the newest timeframe can be ongoing"
      ),
  ...
  private fun startInPresentOrPast(timeframe: Timeframe): Boolean =
    !timeframe.start.isAfter(LocalDate.now())

  private fun endInPresentOrPast(timeframe: Timeframe): Boolean =
    (timeframe.end?.isBefore(LocalDate.now()) ?: true) || (timeframe.end?.isEqual(LocalDate.now())
      ?: true)

  private fun timeframesConsecutive(
  ...
  private fun timeframesNotOverlap(timeframes: List<Timeframe>): Boolean {
    return when {
      ...
      else -> timeframes.first()
        .isOverlappingWith(timeframes[1]) && timeframesNotOverlap(timeframes.drop(1))
    }
  }
